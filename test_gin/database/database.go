...
func RunMigrations(migrations Migrations) {
	...
	for _, model := range migrations.Models {
		...
		/**
		This line has been added to create a test user with name "test" and email "test@goeasycli".
		This is just for testing purposes and should be removed in production.
		*/
		if err == nil && migrations.DB.Migrator().HasTable(&models.GoEasyCLITestUser{}) {
			user := models.GoEasyCLITestUser{
				Name:  "test",
				Email: "test@goeasycli.com",
			}
